...
class TestEvery:
    ...
    def test_wire_tag_is_tag(self) -> None:
        sub = Subscription.every(100, "my_timer")
        assert sub.wire_tag == "my_timer"
...
class TestRendererSubscriptions:
    ...
    def test_wire_tag_global(self) -> None:
        for kind, factory in self.FACTORIES:
            sub = factory()
            assert sub.wire_tag == kind, f"Failed for {kind}"

    def test_wire_tag_window_scoped(self) -> None:
        for kind, factory in self.FACTORIES:
            sub = factory(window="editor")
            assert sub.wire_tag == f"{kind}:editor", f"Failed for {kind}"


class TestKeyEquality:
    def test_same_kind_same_key(self) -> None:
        a = Subscription.on_key_press()
        b = Subscription.on_key_press()
        assert a.key == b.key

    def test_same_kind_different_window(self) -> None:
        a = Subscription.on_key_press()
        b = Subscription.on_key_press(window="editor")
        assert a.key != b.key

    def test_different_kind(self) -> None:
        a = Subscription.on_key_press()
        b = Subscription.on_key_release()
        assert a.key != b.key

    def test_max_rate_does_not_affect_key(self) -> None:
        a = Subscription.on_key_press()
        b = Subscription.on_key_press(max_rate=60)
        assert a.key == b.key


class TestWindowScoping:
    def test_window_kwarg(self) -> None:
        sub = Subscription.on_key_press(window="editor")
    ...
    def test_different_windows_different_keys(self) -> None:
        a = Subscription.on_key_press(window="editor")
        b = Subscription.on_key_press(window="preview")
        assert a.key != b.key
