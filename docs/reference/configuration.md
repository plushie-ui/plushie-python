...
| `bin_file` | `str` | `None` | Override the destination path for the native binary. Without it, downloads land in the platform's standard location. |
| `wasm_dir` | `str` | `None` | Override the output directory for WASM artifacts. Useful for web projects that serve `static/`. |
